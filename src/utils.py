...
# [Sửa đổi hàm select_embedding]
def select_embedding(args, num_nodes, device):
    if args.train_node_embedding:
        emb = torch.nn.Embedding(num_nodes, args.hidden_channels).to(device)
        # --- CODE MỚI: Orthogonal Initialization ---
        if hasattr(args, 'orthogonal_init') and args.orthogonal_init:
            torch.nn.init.orthogonal_(emb.weight)
            print("Đã khởi tạo Node Embeddings theo phương pháp Trực giao (Orthogonal).")
        # -------------------------------------------
        else:
            # Giữ lại cách khởi tạo cũ của ELPH (Xavier Uniform)
