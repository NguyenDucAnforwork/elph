...
def validate_elph(model, val_loader, args, device):
    """
    Calculate validation loss for ELPH model
    """
    model.eval()
    total_loss = 0
...
def validate_generic(model, val_loader, args, device, emb=None):
    """
    Calculate validation loss for SEAL/BUDDY models
    """
    model.eval()
    total_loss = 0
        ...
        for data in val_loader:
            if args.model == 'BUDDY':
                ...
                logits = model(data.z, data.edge_index, data.batch, x, edge_weight, node_id, 
                             data.src_degree, data.dst_degree)
                loss = get_loss(args.loss)(logits, data.y)
                total_loss += loss.item() * args.batch_size
                
    return total_loss / len(val_loader.dataset)

def get_validate_func(args):
    if args.model == 'ELPH':
