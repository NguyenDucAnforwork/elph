...
class LinkPredictor(torch.nn.Module):
    def __init__(self, args, use_embedding=False):
        ...
        # --- SỬA ĐỔI: Thay thế self.lin bằng MLP sâu hơn ---
        self.num_layers = getattr(args, 'predictor_layers', 1) # Mặc định là 1 nếu không truyền
        
        if self.num_layers > 1:
            mlp_modules = []
            # Các lớp ẩn
            for _ in range(self.num_layers - 1):
                mlp_modules.append(Linear(out_channels, out_channels))
                mlp_modules.append(torch.nn.ReLU())
                mlp_modules.append(torch.nn.Dropout(p=args.label_dropout)) # Hoặc tạo dropout riêng
            # Lớp cuối cùng
            mlp_modules.append(Linear(out_channels, 1))
            self.lin = torch.nn.Sequential(*mlp_modules)
        else:
            self.lin = Linear(out_channels, 1) # Fallback về shallow
...
class ELPH(torch.nn.Module):
    ...
    def feature_conv(self, x, edge_index, k, x0=None):
    ...
    def embedding_conv(self, x, edge_index, k, x0=None):
        if x is None:
            return x
